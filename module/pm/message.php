...
// $Info Суҳбати ду нафарро пайдо  кард! ва дар худаш суҳбати ҳамон нафарҳоро ID - ашонро дорад!
...
//Фиристодаги касе бошад мо логенашро гирифтем. ТАНҲО АЗ ЯК НАФАРРО!
...
<div class="Page">
<?php
	// $Query - ҳамон маълумоте мегирад, ки ID-аш баробари ID-и диалоги саҳифаи мо бошад!
	$Query = mysqli_query($CONNECT, "SELECT * FROM `message` WHERE `did` = $Param[id] ORDER BY `id`");

	while ($Row = mysqli_fetch_assoc($Query)) {
		//Логени интиқолдиҳандаро ёфт кардем ва ба $Row['user'] бахшидем
		if ($Info['send'] == $Row['user']) $Row['user'] = $User['login'];
		else $Row['user'] = $_SESSION['USER_LOGIN'];
		echo '
			<div class="ChatBlock">
				<span>'.$Row['date'].' от '.$Row['user'].'</span>
				'.$Row['text'].'
			</div>';
	}
?>
